...
public class MqttEncoder extends MessageToMessageEncoder<MqttPacket> {
  ...
  private static byte[] PROTOCOL_NAME_BYTES =
      ValidationUtils.validateAndEncodeString("MQTT", "protocol name");
  ...
  private static ByteBuf encodeConnect(
      ByteBufAllocator byteBufAllocator, @NonNull MqttConnectPacket packet) {
    ...
    val fixedHeaderSize =
        1 + getAndValidateVariablePartLengthSize(variableHeaderSize + variablePartSize);
  ...
  private static ByteBuf encodePingReq(
      ByteBufAllocator byteBufAllocator, @NonNull MqttPingReqPacket packet) {
  ...
  private static ByteBuf encodeSubscribe(
      ByteBufAllocator byteBufAllocator, @NonNull MqttSubscribePacket packet) {
    val topicFilterBytesList = new LinkedList<byte[]>();
    var variablePartSize = 2;
    var payloadCount = 0;
    for (MqttTopicAndQosLevel payload :
        ValidationUtils.requireNonNull(packet.getMqttTopicAndQosLevels(), "topic and qos list")) {
      byte[] topicFilterBytes =
          ValidationUtils.validateAndEncodeTopicFilter(payload.getTopicFilter());
      if (payload.getQosLevel() == MqttQosLevel.FAILURE) {
        throw new EncoderException(
            "[MQTT-3.3.1-4] PUBLISH Packet MUST NOT have both QoS bits set to 1");
      }
  ...
  private static ByteBuf encodePublish(
      ByteBufAllocator byteBufAllocator, @NonNull MqttPublishPacket packet) {
  ...
  private static ByteBuf encodeDisconnect(
      ByteBufAllocator byteBufAllocator, @NonNull MqttDisconnectPacket packet) {
    val buf = byteBufAllocator.buffer(2);
    buf.writeByte(0xE0);
    buf.writeByte(0);
    return buf;
  ...
  private static ByteBuf encodeConnAck(
      ByteBufAllocator byteBufAllocator, @NonNull MqttConnAckPacket packet) {
  ...
  private static ByteBuf encodeSubAck(
      ByteBufAllocator byteBufAllocator, @NonNull MqttSubAckPacket packet) {
  ...
  private static ByteBuf encodeUnsubscribe(
      ByteBufAllocator byteBufAllocator, @NonNull MqttUnsubscribePacket packet) {
    var variablePartSize = 2;
  ...
  private static ByteBuf encodeUnsubAck(
      ByteBufAllocator byteBufAllocator, @NonNull MqttUnsubAckPacket packet) {
    val buf = byteBufAllocator.buffer(4);
    buf.writeByte(0xB0);
    buf.writeByte(0x02);
    buf.writeShort(packet.getPacketId());
  ...
  private static ByteBuf encodePingResp(
      ByteBufAllocator byteBufAllocator, @NonNull MqttPingRespPacket packet) {
    val buf = byteBufAllocator.buffer(2);
    buf.writeByte(0xD0);
    buf.writeByte(0);
    return buf;
